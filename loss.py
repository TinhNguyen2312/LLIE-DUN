...
# --- FDU-Net specific terms (Fourier-domain + intermediate supervision) ---


class FourierDomainLoss(nn.Module):
    # MSE on real/imag parts of final DFT vs ground truth (lambda_1, lambda_2)
    def __init__(self, loss_weight_real: float = 1.0, loss_weight_imag: float = 1.0):
        super().__init__()
        self.loss_weight_real = loss_weight_real
        self.loss_weight_imag = loss_weight_imag

    def forward(self, x_pred, y_pred, x_gt, y_gt):
        real_loss = F.mse_loss(x_pred, x_gt)
        imag_loss = F.mse_loss(y_pred, y_gt)
        total = self.loss_weight_real * real_loss + self.loss_weight_imag * imag_loss
        return total, real_loss, imag_loss


class IntermediateSupervisionLoss(nn.Module):
    # per-stage L1 supervision (lambda_3), enforces monotone FISTA descent
    def __init__(self, loss_weight: float = 1.0, increasing_weight: bool = False):
        super().__init__()
        self.loss_weight = loss_weight
    ...
    def forward(self, intermediates, target):
        ...
        if self.increasing_weight:
            stage_weights = torch.linspace(
                0.5, 1.5, steps=num_stages, device=target.device
            )
            stage_weights = stage_weights / stage_weights.sum() * num_stages
        else:
            stage_weights = torch.ones(num_stages, device=target.device)

        per_stage_losses = [
            stage_weights[k] * F.l1_loss(inter, target)
            for k, inter in enumerate(intermediates)
        ]
        mean_loss = torch.stack(per_stage_losses).mean()
        return self.loss_weight * mean_loss, mean_loss


class FDULoss(nn.Module):
    # wraps LowLightLoss (pixel-space, unchanged) + Fourier + intermediate terms
    # fourier/intermediate disabled by default (weight 0), so legacy configs still work
    def __init__(self, loss_weights: Dict, **kwargs):
        super().__init__()
        self.weights = loss_weights
        self.pixel_loss = LowLightLoss(loss_weights, **kwargs)

        fourier_real_w = loss_weights.get("fourier_real", 0.0)
        fourier_imag_w = loss_weights.get("fourier_imag", 0.0)
        self.fourier_enabled = (fourier_real_w > 0) or (fourier_imag_w > 0)
        self.fourier_loss = (
            FourierDomainLoss(fourier_real_w, fourier_imag_w)
            if self.fourier_enabled
            else None
        )

        intermediate_w = loss_weights.get("intermediate", 0.0)
        self.intermediate_enabled = intermediate_w > 0
        self.intermediate_loss = (
            IntermediateSupervisionLoss(
                loss_weight=intermediate_w,
                increasing_weight=loss_weights.get(
                    "intermediate_increasing_weight", False
                ),
            )
            if self.intermediate_enabled
            else None
        )

    def forward(self, pred, target, model=None, intermediates=None):
        loss_dict = self.pixel_loss(pred, target)
        # LowLightLoss returns plain 0 (not a tensor) for disabled sub-losses
        for key, value in list(loss_dict.items()):
            if not torch.is_tensor(value):
                loss_dict[key] = pred.new_tensor(float(value))
        total = loss_dict["total"]

        fourier_loss_val = pred.new_tensor(0.0)
        if self.fourier_enabled:
            if model is None:
                raise ValueError("FDULoss: fourier loss enabled but no model passed.")
            x_pred, y_pred = model.get_fourier_prediction(pred)
            x_gt, y_gt = model.get_fourier_targets(target)
            fourier_loss_val, _, _ = self.fourier_loss(x_pred, y_pred, x_gt, y_gt)
            total = total + fourier_loss_val

        intermediate_loss_val = pred.new_tensor(0.0)
        if self.intermediate_enabled:
            if intermediates is None:
                raise ValueError(
                    "FDULoss: intermediate loss enabled but no intermediates passed."
                )
            intermediate_loss_val, _ = self.intermediate_loss(intermediates, target)
            total = total + intermediate_loss_val

        loss_dict["total"] = total
        loss_dict["fourier"] = fourier_loss_val
        loss_dict["intermediate"] = intermediate_loss_val
        return loss_dict
